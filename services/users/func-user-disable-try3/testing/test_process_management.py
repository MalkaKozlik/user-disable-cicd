...
from project.process_management import (
    get_access_token,
    check_user_expiration_date,
    send_users_for_testing,
...
def test_check_user_expiration_date_assert_called_check_if_x_days_passed_since_expiration_date(
...
def test_check_user_expiration_date_assert_called_add_property_to_user(
    update_user_attribute,
):
    check_user_expiration_date("access_token", {"id": "id"}, 3, "application_id")
...
@patch(
    "project.process_management.users_by_department",
    Mock(return_value=[{"id": "aaa"}, {"id": "bbb"}]),
)
@patch("project.process_management.check_user_expiration_date")
def test_send_users_for_testing_called_check_user_expiration_date(
    check_user_expiration_date,
):
    send_users_for_testing("access_token", "department", 3, "application_id")
    assert check_user_expiration_date.call_count == 2
...
@patch("project.process_management.send_users_for_testing")
def test_inspection_process_management_assert_called_send_users_for_testing(
    send_users_for_testing,
):
    inspection_process_management()
    send_users_for_testing.called_assert_once_with(
...
def test_inspection_process_management_return_failed():
    assert inspection_process_management() == "Failed to obtain access token"
...
@patch(
    "project.process_management.send_users_for_testing",
    Mock(side_effect=Exception()),
)
def test_inspection_process_management_return():
    assert inspection_process_management() == "send_users_for_testing does not succeed"
...
@patch("project.process_management.get_kv_value_secret")
def test_get_access_token_and_app_id(get_kv_value_secret):
    get_access_token_and_app_id()
    calls = [
    ...
    get_kv_value_secret.assert_has_calls(calls)
...
def test_get_kv_value_secret():
    assert get_kv_value_secret(client(), "secret_name") == ("secret")
